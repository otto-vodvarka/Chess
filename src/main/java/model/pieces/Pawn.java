...
public class Pawn extends Piece {
    ...
    public List<Move> getAllAvailableMoves(Board board) {
        ...
        for (int i = -1; i <= 1; i++) {
            //white
            Move moveWhite = new Move(myCoord, new Coordinate(myCoord.getX() + i, myCoord.getY() - 1));
            if (moveWhite.isAtBoard() && isMoveValid(board, moveWhite)) {
                moves.add(moveWhite);
            }
            
            //black
            Move moveBlack = new Move(myCoord, new Coordinate(myCoord.getX() + i, myCoord.getY() + 1));
        ...
        //two steps forward
        //white
        Move moveWhite = new Move(myCoord, new Coordinate(myCoord.getX(), myCoord.getY() - 2));
        if (moveWhite.isAtBoard() && isMoveValid(board, moveWhite)) {
            moves.add(moveWhite);
        }
        //black
        Move moveBlack = new Move(myCoord, new Coordinate(myCoord.getX(), myCoord.getY() + 2));
        if (moveBlack.isAtBoard() && isMoveValid(board, moveBlack)) {
            moves.add(moveBlack);
        }
        return moves;
